...
void ANN::testing(vector< vector<float> > X) {
	//cout << "----------------------------" << endl;
...
void ANN::trainSet(float r, int b, int s) {
	this->learningRate = r;
	this->batchSize = b;
	this->stopEpochs = s;
}

void ANN::weightSet(vector< vector<float> > X, int numOfHiddenLayer, int numOfNode) {
...
float ANN::sigmoid(float toSigmoid) {
	return (1 / (1 + exp(-toSigmoid)));
}

float ANN::dsigmoid(float Sig) {
	return Sig * (1 - Sig);
}
...
vector<float> ANN::scalarV(vector<float> V, float Snum) {
	for (int i = 0; i < V.size(); i++) {
		V[i] = V[i] * Snum;
		//cout << V[i];
	}
...
vector<float> ANN::v_v(vector<float> V1, vector<float> V2) {  //vector multipication (vector)
	vector<float> Mat;
	Mat.resize(V1.size());
...
vector<vector<float> > ANN::v_v_m(vector<float> V1, vector<float> V2) {  //vector multipication (output matrix)
...
vector < vector<float>> ANN::transpose(vector<vector<float> >& m) {  //matrix transpose
...
vector < vector<float>> ANN::m_m(vector< vector<float> > A, vector< vector<float> > B)  //matrix multiplication
{
	float a = 0;
...
vector<float> ANN::m_v(vector< vector<float> > X, vector<float> Y)
{
	vector<float> V;
	V.resize(X.size());
...
vector<int>  ANN::feedForward(vector< vector<float> > X) {
	vector<float> outPut;
	outPut.resize(10);
	...
	for (int totalData = 0; totalData < X.size(); totalData++) {
		hiddenLayer_F[totalData][0] = m_v(firstWeight, X[totalData]);
		for (int i = 0; i < hiddenLayer_F[totalData][0].size(); i++) {
...
void ANN::train(vector< vector<float> > X, vector<float> y) {
...
void ANN::inference(vector<float> X) {
...
void ANN::storeWeight(string fileName) {
	ofstream outFile;
...
void ANN::loadWeight(string fileName) {
	ifstream inFile;
